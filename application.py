...
def card(data):
    ...
    # Player 2
        ...
        # Alderaan
            ...
            db.execute(f"UPDATE games SET player1_credits = ?, player2_credits = ?, hand_pot = ?, phase = ?, player2_card = ?, player_turn = ?, completed = ? WHERE game_id = {game_id}", game["player1_credits"] - (game["hand_pot"] * p1BombOut), "completed", player2_hand, action, -1, 1)
            game = db.execute(f"SELECT * FROM games WHERE game_id = {game_id}")[0]
