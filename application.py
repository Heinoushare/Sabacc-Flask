...
def bet(data):
    ...
    if action == "bet" and player == "player1" and game["player_turn"] == game["player1_id"] and amount >= 0 and amount <= game["player1_credits"]:
        ...
        db.execute(f"UPDATE games SET player1_credits = ?, player1_bet = ?, player2_bet = ?, hand_pot = ?, phase = ?, player_turn = ? WHERE game_id = {game_id}", game["player1_credits"] - ["player2_bet"], None, None, game["hand_pot"] + (amount * 2) - game["player1_bet"], "card", game["player1_id"])
